...
export default function Barchart(combinedWeekData) {
  const chartContainerRef = useRef(null);

  const data = {
    labels: combinedWeekData.combinedWeekData.map((item) => item.week), // e.g., ['37', '40', '41', ...]
    datasets: [
      {
        label: "Planned",
        data: combinedWeekData.combinedWeekData.map((item) => item.planned),
        backgroundColor: "rgba(75, 192, 192, 0.6)",
      },
      {
        label: "Actual",
        data: combinedWeekData.combinedWeekData.map((item) => item.actual),
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: "Planned Vs Actual (Week Wise)",
      },
    },
  };
