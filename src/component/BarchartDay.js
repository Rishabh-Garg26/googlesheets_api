...
export default function Barchart(combinedDayData) {
  const chartContainerRef = useRef(null);

  const data = {
    labels: combinedDayData.combinedDayData.map((item) => item.date), // e.g., ['37', '40', '41', ...]
    datasets: [
      {
        label: "Planned",
        data: combinedDayData.combinedDayData.map((item) => item.planned),
        backgroundColor: "rgba(75, 192, 192, 0.6)",
      },
      {
        label: "Actual",
        data: combinedDayData.combinedDayData.map((item) => item.actual),
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: "Planned Vs Actual (Day Wise)",
      },
    },
  };
